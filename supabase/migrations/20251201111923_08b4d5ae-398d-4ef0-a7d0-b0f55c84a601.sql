-- Create view for video details with creator and engagement metrics
CREATE OR REPLACE VIEW public.video_details_view AS
SELECT 
  v.id,
  v.title,
  v.description,
...
-- Create view for user playlists with video counts
CREATE OR REPLACE VIEW public.user_playlists_view AS
SELECT 
  pl.id,
  pl.user_id,
  pl.title,
...
-- Create view for watch history with full video details
CREATE OR REPLACE VIEW public.watch_history_details_view AS
SELECT 
  wh.id,
  wh.user_id,
  wh.video_id,
...
-- Create view for trending videos (high engagement)
CREATE OR REPLACE VIEW public.trending_videos_view AS
SELECT 
  v.id,
  v.title,
  v.description,
  ...
  -- Engagement score: views + (likes * 10) + (comments * 5) + (favorites * 3) - (dislikes * 5)
  v.views + 
...
GRANT SELECT ON public.video_details_view TO authenticated;
GRANT SELECT ON public.user_playlists_view TO authenticated;
GRANT SELECT ON public.comment_details_view TO authenticated;
GRANT SELECT ON public.watch_history_details_view TO authenticated;
GRANT SELECT ON public.trending_videos_view TO authenticated;
GRANT SELECT ON public.user_statistics_view TO authenticated;

-- Add comments explaining the views
COMMENT ON VIEW public.video_details_view IS 'Combines videos with creator info and engagement metrics (likes, dislikes, comments, favorites)';
COMMENT ON VIEW public.user_playlists_view IS 'Shows playlists with video counts and creator information';
COMMENT ON VIEW public.comment_details_view IS 'Comments joined with user profile information';
COMMENT ON VIEW public.watch_history_details_view IS 'Watch history with complete video and creator details';
COMMENT ON VIEW public.trending_videos_view IS 'Videos with engagement score for trending/popular content';
COMMENT ON VIEW public.user_statistics_view IS 'User profile statistics including videos, views, playlists, and activity';